  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200">
          <Link href="/dashboard/merchant" className="flex items-center gap-2">
            <Store size={22} style={{ color: '#E23744' }} />
            <span className="font-bold text-lg text-gray-900">Merchant</span>
          </Link>
          <button onClick={closeSidebar} className="lg:hidden p-1 rounded-lg hover:bg-gray-100 text-gray-500">
            <X size={20} />
          </button>
        </div>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-200 h-16 flex items-center px-4 sm:px-6">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg hover:bg-gray-100 text-gray-600 mr-3">
            <Menu size={20} />
          </button>
          <div className="flex-1" />
          <Link href="/dashboard/merchant/notifications" className="relative p-2 rounded-lg hover:bg-gray-100 text-gray-600">
            <Bell size={20} />
          </Link>
        </header>
